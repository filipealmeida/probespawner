...
#import java.net.InetAddress as InetSocketTransportAddress
...
#import com.google.common.collect.ImmutableSet as ImmutableSet
...
class Elasticsearch():
    def __init__(self, config):
        ...
        self.config["settings"] = Settings.settingsBuilder();
        ...
        self.runtime["client"] = TransportClient.builder().settings(self.config["settings"]).build()
        if "host" in self.config:
            #self.runtime["client"].addTransportAddress(InetSocketTransportAddress(host, port))
			self.runtime["client"].addTransportAddress(InetSocketTransportAddress(InetAddress.getByName(host), port))
        if "hosts" in self.config:
            for hostport in self.config["hosts"]:
                ...
                self.runtime["client"].addTransportAddress(InetSocketTransportAddress(InetAddress.getByName(str(host)), int(port)))
